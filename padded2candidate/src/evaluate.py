...
class UserDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # if len(item["clicked_news"]) == 0:
        #     item["clicked_news"].extend(['PADDED_NEWS'])
        return item
    ...
    try:
        with tqdm(total=len(behaviors_dataloader),
            ...
            for minibatch in behaviors_dataloader:
                ...
                if generate_txt:
                    answer_file.write(
                        f"{minibatch['impression_id'][0]} {user2length[minibatch['clicked_news_string'][0]]} {str(value2rank(y_pred_list)).replace(' ','')}\n"
                    )
                pbar.update(1)
